...
def sv_measurement_along(v_dir, v_a, v_b, x1, x2, zc=2.5):
    """
    Measure the metric length of segment (x1,x2) aligned to v_dir, using the vanishing line from (v_a, v_b).
    All inputs are 3D homogeneous *image* directions/points in the normalized camera frame (i.e., after invK).
    Mirrors your sv_measurement(...) but works for ANY axis by permutation of VPs.
    """
    vline = np.cross(v_a, v_b)                 # vanishing line orthogonal to v_dir
    p4 = vline / (np.linalg.norm(vline) + 1e-8)

    # scale using the same convention as your sv_measurement()
...
def heightCalc(fname_dict, intrins, config, img_size=None, pitch=None, use_pitch_only=0, use_detected_vpt_only=0, verbose=False):
    ...
    try:
        ...
        # classify lines; now returns verticals + two horizontal groups (aligned with v1_right, v2_left)
        verticals, hori0_lines, hori1_lines = filter_lines_outof_building_ade20k(
        img_fname, line_segs, scores, seg_img, vps, config, use_vertical_vpt_only=use_pitch_only, verbose=verbose
        )
        ...
        wd_set = []   # entries like [width_m, a, b, group_id]
    ...
    # lines consistent with v1_right → use vanishing line from (v2_left, v3_vertical)
    add_widths_from(hori0_lines, vps0_d3, vps1_d3, vps2_d3, grp_id=0)

    # lines consistent with v2_left → use vanishing line from (v1_right, v3_vertical)
    ...
    if verbose:
        ...
        grouped_widths = None
        if len(wd_set) > 0:
          grouped_widths = clausterLinesWithCenters(wd_set, config, using_height=True)  # same API: [val,a,b,...,median,mean]
...
def _centroid_of_group(grp):
    # grp: list of [val, a, b, ...] then [median, mean] at end
    pts = []
    for it in grp[:-2]:
        _, a, b, *_ = it
...
if grouped_widths is not None:
    # naive 1-1 pairing by nearest centroid in image space
    h_centers = [ _centroid_of_group(g) for g in grouped_lines ]
    w_centers = [ _centroid_of_group(g) for g in grouped_widths ]

    for hi, hg in enumerate(grouped_lines):
        if not np.isfinite(h_centers[hi]).all(): continue
        h_med, h_mean = hg[-2], hg[-1]
        # find nearest width group
        dists = [ np.linalg.norm(h_centers[hi] - wc) if np.isfinite(wc).all() else 1e9 for wc in w_centers ]
        if len(dists)==0 or min(dists) == 1e9: continue
        wi = int(np.argmin(dists))
        wg = grouped_widths[wi]
        w_med, w_mean = wg[-2], wg[-1]

        area_med = h_med * w_med
    ...
    for k, a in enumerate(areas):
        print(f"  #{k}: H_med={a['height_median']:.3f} m, W_med={a['width_median']:.3f} m,  "
              f"Area_med={a['area_median']:.3f} m^2  (pair_dist_px={a['pair_dist_px']:.1f})")
else:
