...
def _seg_dir(p1, p2):          # (y,x) inputs
    ...
    n  = (vx**2 + vy**2) ** 0.5 + 1e-8
    return np.array([vx/n, vy/n], float)

def _angle_to_vp(p1, p2, vp):
    # vp=(x,y). Note (y,x) vs (x,y) carefully.
    m  = np.array([(p1[0]+p2[0])/2.0, (p1[1]+p2[1])/2.0])  # (y,x)
    d  = _seg_dir(p1, p2)                                  # [dx, dy]
    v  = np.array([vp[0]-m[1], vp[1]-m[0]])                # (x - mx, y - my)
    nv = np.linalg.norm(v) + 1e-8
    v /= nv
    cosang = np.clip(abs(d[0]*v[0] + d[1]*v[1]), 0, 1)
    return np.degrees(np.arccos(cosang))

def classify_lines(lines, scores, vps2d, building_mask, min_score=0.0,
                   tol_vert_deg=10.0, tol_horiz_deg=10.0):
    """
    lines: (N,2,2) [[y1,x1],[y2,x2]]
    vps2d: dict with keys: 'v1_right','v2_left','v3_vertical' (x,y) in px
    building_mask: HxW bool
    """
    H, W = building_mask.shape
    out = dict(vertical_idx=[], horizontal_idx=[])
    v1 = tuple(vps2d["v1_right"]); v2 = tuple(vps2d["v2_left"]); v3 = tuple(vps2d["v3_vertical"])

    for i, (p1, p2) in enumerate(lines):
        if scores[i] < min_score:
            continue
        # midpoint inside building?
        my = int(round((p1[0]+p2[0])/2.0)); mx = int(round((p1[1]+p2[1])/2.0))
        if not (0 <= my < H and 0 <= mx < W and building_mask[my, mx]):
            continue
        ...
        if min(ang_h1, ang_h2) <= tol_horiz_deg:
